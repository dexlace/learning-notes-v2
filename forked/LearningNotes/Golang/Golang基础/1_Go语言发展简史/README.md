...
Go语言保证了既能到达==静态编译语言的安全和性能==，又达到了==动态语言开发维护的高效率==，使用一个表达式来形容Go语言：==Go=C+Python==，说明==Go语言既有C静态语言程序的运行速度==，又能==达到Python动态语言的快速开发==。

- 从c语言中继承了很多理念，包括==表达式语法，控制结构，基础数据类型，调用参数传值，指针等等==，也保留了和C语言一样的编译执行方式及弱化的指针。
...
- 如果我们==先编译生成了可执行文件==，那么我们可以将该可执行文件拷贝到==没有go开发环境的机器上，然可以运行==
- 如果我们是==直接go rungo源代码==，那么如果要在另外一个机器上这么运行，也==需要go开发环境==，否则无法执行。
...
- Go语言==严格区分大小写==。
- Go方法由一条条语句构成，每个语句后==不需要分号==（Go语言会在每行后自动加分号），这也体现出Golang的简洁性。
- Go编译器是一行行进行编译的，因此我们==一行就写一条语句==，不能把多条语句写在同一个，否则报错
