...
每个变量在运行时都拥有一个地址，这个地址代表变量在内存中的位置。Go 语言中==使用&字符放在变量前面对变量进行取地址操作==。Go语言中的值类型（int、float、bool、string、array、struct）都有对应的指针类型，如：
...
这个时候，我们就需要==使用new关键字来分配内存，new是一个内置的函数==，它的函数签名如下：
...
- 而==new用于类型的内存分配==，并且内存赌赢的值为类型的零值，返回的是指向类型的指针