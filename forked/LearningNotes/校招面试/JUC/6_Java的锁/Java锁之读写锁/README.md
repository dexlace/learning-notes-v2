...
对ReentrantReadWriteLock其==读锁是共享==，其==写锁是独占==

写的时候只能一个人写，但是读的时候，可以多个人同时读

## 为什么会有写锁和读锁

原来我们使用ReentrantLock创建锁的时候，是独占锁，也就是说一次只能一个线程访问，但是==有一个读写分离场景==，读的时候想同时进行，因此原来独占锁的并发性就没这么好了，因为读锁并不会造成数据不一致的问题，因此可以多个人共享读
